...
- **長い動画の解析中に Mac がアイドルスリープすると、検証・注釈が `TypeError: Invalid state: Controller is already closed` で失敗する。** 実際に発生した事例（`pmset -g log` で確認）: 解析中に約7分間システムスリープに入り、復帰後に (1) スリープ中も経過扱いになる `setTimeout` のせいで `postJson` の300秒タイムアウトが復帰直後に一斉発火（`annotate` 側の `TimeoutError`）、(2) スリープ中にブラウザ側の SSE 接続が切れていたのに、サーバ側の `mapWithConcurrency` はまだ動いていて、閉じられた `controller` に `enqueue` しようとして例外、という2つの症状が連鎖した。ワークフロー表示（`WorkflowHeader.tsx`）はこれを正しく「✕ 検証」として拾えるようになっているが、根本原因はネットワークやコードのバグではなくスリープによる接続断なので、**長時間の動画解析を行う前に `caffeinate` などでスリープを防ぐ**運用で対処する。
